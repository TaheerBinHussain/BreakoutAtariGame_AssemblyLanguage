...
start_game:
    call init_game ; bricks initialization and score initialization
    call clear_screen
...
game_loop:
    ...
    ; Read the key (clears buffer)
    mov ah,0
    int 16h
    
    cmp al,1Bh ;buffer to al
    jne check_space_key
    jmp back_to_menu
...
update_ball1: ;when no key is pressed
    call delay_ball ; Total Delay = Ball Speed * 16,384 (0x4000)
    call clear_ball
    
    mov al,[ball_x]
    add al,[ball_dx] ;adding x coordinate of ball with horizontal speed of the ball (balls moves right)
    mov [ball_x],al ;updating x coordinate of the ball
    
    mov al,[ball_y]
    add al,[ball_dy] ;adding y coordinate of ball with horizontal speed of the ball (ball moves up)
    mov [ball_y],al ;updating y coordinate of the ball
...
move_left:
    ...
    cmp byte [ball_active],0 ;inactive -> clear ball
...
move_left_done:
    call draw_paddle ;clear and draw paddle
...
next_level:
    ...
    mov al,[ball_speed] ;ball speed used only in delay function
    cmp al,4 ; initial ball speed = 14 ouuter loop (ball speed) (0x4000)
...
init_game:
    ...
    mov byte [powerup_active],0  ;attributes
    mov byte [double_points_active],0
    mov byte [double_points_timer],0
...
init_level:
    ...
    mov si,brick_data ; si=brick data array
    mov di,brick_scores ; di=brick score array =0
    mov cx,40 ; cx=number of bricks
    xor bx,bx ; bx=0
...
init_level_loop:
    ...
    mov ax,bx ;ax=bx
    mov dl,10
    div dl ; ah=remainder and al contains the quotient
    
    cmp al,0
    jne init_check_row1
    mov byte [di],4 ; brick stored on row got the score 4 
    jmp init_next_brick
...
init_next_brick: ;loop condition for init_level
    inc si
    inc di
    inc bx
    loop init_level_loop ;cx times (40)
    ret
...
check_wall_hit_right:
    neg byte [ball_dx] ;change the speed sign
    mov byte [ball_x],78
...
check_wall_check_top:
    ...
    cmp al,1
    jg check_wall_done
    neg byte [ball_dy] ;ball vertical speed sign changes as we want the ball to move down now
    mov byte [ball_y],1
check_wall_done:
    ret

check_paddle_collision:
    mov al,[ball_y] ;ball row
    mov bl,[paddle_y] ;paddle row
    cmp al,bl
    jne check_paddle_no_collision
    
    mov al,[ball_dy] ;The Direction Check (Is it falling?)
    cmp al,0
    ...
    mov bl,[paddle_x]
    cmp al,bl
    jl check_paddle_no_collision ; if ball is left to the paddle then no collision
    ...
    neg byte [ball_dy] ; ball moves up now
    call sound_paddle_hit
    
    mov al,[ball_y]
    dec al
    mov [ball_y],al ; ball goes 1 row up
    
    mov al,[ball_x]
    sub al,[paddle_x]
    mov bl,[paddle_width]
    shr bl,1 ; (8/2) first finds half of the paddle length
    
    cmp al,bl ; compare this to find out where the ball hit (whether to the left from mid of paddle or right)
...
check_brick_collision:
    mov al,[ball_y]
    cmp al,2
    jge check_brick_y_ok1       ; Changed from jl
    jmp check_brick_no_hit
check_brick_y_ok1:
    cmp al,6
    jl check_brick_y_ok2        ; Changed from jge
    jmp check_brick_no_hit
check_brick_y_ok2:
    ...
    mov al,[ball_x]
    sub al,5
    cmp al,0 ; Subtract the Left Margin.
             ; The bricks don't start at X=0; they start at X=5.
             ; We need to know how far the ball is inside the brick area.

    jge check_brick_x_ok        ; Changed from jl
    jmp check_brick_no_hit
check_brick_x_ok:
    ...
    div cl ; Divide AX by CL.
           ; Math: (Ball Relative X) / 7
           ; Result: 
           ;   AL = Quotient (The Column Number)
    
    cmp al,10
    jl check_brick_col_ok       ; Changed from jge
    jmp check_brick_no_hit
check_brick_col_ok:
    
    xor ah,ah
    add ax,bx ; ax(brick_number) + bx (row*10)  bx calculated above
    
    cmp ax,40
    jl check_brick_idx_ok       ; Changed from jge
    jmp check_brick_no_hit
check_brick_idx_ok:
...
check_brick_exists:
    ...
    mov si,brick_scores
    add si,bx
    mov al,[si] ;al contains the brick score
...
draw_game_border: ;main game border
    mov dh,0 ; y=0 (rows)
    mov dl,0 ; x=0 (columns)
    mov al,'+'
    mov bl,[color_border] ;color_border = yellow
    call print_color_char
    mov dl,1
    mov cx,78
draw_border_top_loop: ; (printing = on the top row)
    push cx
    mov al,'='
    mov bl,[color_border]
    call print_color_char
...
side_continue_border: ; printing side border on both sides
    mov dl,0
    mov al,'|'
    mov bl,[color_border]
    call print_color_char
...
side_done_border:
    ...
    mov dl,1
    mov cx,78
draw_border_bottom_loop: ;draws bottom border
    push cx
    mov al,'='
    mov bl,[color_border]
    call print_color_char
...
draw_bricks:
    ...
    mov si,brick_data
    mov dh,2 ;starting row for bricks
    mov cl,0 ;row number
    
draw_bricks_row_loop:
    cmp cl,4
    jge draw_bricks_done
    
    mov al,cl
    cmp al,0
    jne check_row1_pre
    mov bl,0x0C ;brick color (red for first row)
    jmp draw_row_start
check_row1_pre:
    cmp al,1
    jne check_row2_pre
...
draw_row_start:
    mov [temp_color],bl
    mov dl,5 ;starting col of brick
    mov ch,0
    
draw_bricks_col_loop:
    cmp ch,10 ; ch contains the bricks made for a specific row (ch = 0-9 bricks in a row)
    jge draw_bricks_next_row
    ...
    mov al,0DBh      ; Character: 0DBh is the Solid Block symbol
    mov bl,[temp_color]
    push cx
    mov cx,7 ;length of the brick
    int 10h
    pop cx
...
draw_paddle_clear_loop:
    ...
    mov ah,09h
    mov al,0DBh ;ASCII code for a Full Solid Block
    mov bl,0x09 ;Light Blue text on a Black background
    xor ch,ch
    mov cl,[paddle_width] ; paddle size
    int 10h
...
draw_score:
    ...
    mov si,score_label
    mov al,[color_desc] ; color for score 
    mov [current_color],al
    call print_color_string
...
print_color_string:
    ...
    mov bh,0
    int 10h
    ;gets the cursor current location through dx
...
;Go to a specific spot on the screen, and draw one colored character there
;DL, DH: The X, Y coordinates (Column, Row) where you want to draw.
;AL: The Character you want to draw (e.g., '=', '|', or a block).
;BL: The Color you want (e.g., 0x0E for Yellow).

    push ax
    push bx
    push cx
    
    mov bh,0    ; Page 0 (Main screen)
    mov ah,02h  ; BIOS Command: "Set Cursor Position"
    int 10h     ; Execute
    ...
    mov ah,09h ; BIOS Command: "Write Character & Attribute"
    mov bh,0 ; Page 0
    mov cx,1 ; Repeat count: Draw the character 1 time
    int 10h 
...
logo2 db "   | __ )|  _ \| ____|  / \  | |/ / / _ \| | | |_   _|",0
logo3 db "   |  _ \| |_) |  _|   / _ \ | ' / | | | | | | | | |  ",0
logo4 db "   | |_) |  _ <| |___ / ___ \| . \ | |_| | |_| | | |  ",0
logo5 db "   |____/|_| \_\_____/_/   \_\_|\_\ \___/ \___/  |_|  ",0
...
level db 1 
ball_speed db 14   ;initial ball speed = 14
